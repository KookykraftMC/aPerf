...
public class Module extends BaseCommand {
    ...
    public void list(Object plugin, ICommandSender sender, Map<String, String> args) {
        ...
        for (ModuleBase m : aPerf.instance.modules) {
            ...
            msg(sender, format, m.isEnabled() ? ChatColor.DARK_GREEN : ChatColor.RED, m.isEnabled() ? "yes" : "no", ChatColor.GREEN, aPerf.instance.isEnabled(m) ? ChatColor.DARK_GREEN : ChatColor.RED, aPerf.instance.isEnabled(m) ? "yes" : "no", ChatColor.GREEN, m.getName());
        }

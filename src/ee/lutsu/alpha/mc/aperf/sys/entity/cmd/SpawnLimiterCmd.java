...
public class SpawnLimiterCmd extends BaseCommand {
    ...
    public void list(Object plugin, ICommandSender sender, Map<String, String> args) {
        ...
        msg(sender, "%sExecute onChunkLoad: %s%s%s, normalSpawn: %s%s%s, otherSpawn: %s%s%s", ChatColor.GREEN, SpawnLimiterModule.instance.executeRulesOnChunkLoad ? ChatColor.DARK_GREEN : ChatColor.RED, SpawnLimiterModule.instance.executeRulesOnChunkLoad ? "on" : "off", ChatColor.GREEN,

        SpawnLimiterModule.instance.executeRulesOnNormalSpawning ? ChatColor.DARK_GREEN : ChatColor.RED, SpawnLimiterModule.instance.executeRulesOnNormalSpawning ? "on" : "off", ChatColor.GREEN,

        SpawnLimiterModule.instance.exetuteRulesOnOtherwiseSpawned ? ChatColor.DARK_GREEN : ChatColor.RED, SpawnLimiterModule.instance.exetuteRulesOnOtherwiseSpawned ? "on" : "off", ChatColor.GREEN);
        ...
        for (SpawnLimit limit : SpawnLimiterModule.instance.limits) {
            msg(sender, format, ChatColor.GREEN, i++, limit.on ? ChatColor.DARK_GREEN : ChatColor.RED, limit.on ? "on" : "off", ChatColor.GREEN, limit.type.name(), limit.filter.serializeDisplay(), limit.getDisplayOptions());
        }
    ...
    public void types(Object plugin, ICommandSender sender, Map<String, String> args) throws InstantiationException, IllegalAccessException {
        ...
        for (SpawnLimit.Type t : SpawnLimit.Type.values()) {
            ...
            for (Entry<String, String> kv : targs.entrySet()) {
                ...
                msg(sender, "%s   %s%s - %s", optional ? ChatColor.YELLOW : ChatColor.GOLD, key, ChatColor.GRAY, kv.getValue() + (optional ? "(optional)" : ""));
            }
        }
        msg(sender, "%s-----------------------------------------------------", ChatColor.GRAY);
    }

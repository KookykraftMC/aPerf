...
import aperf.sys.entity.EntityClassMap;

public class SubFilter {
	public enum Type {
		All("Hits everything", "No value"), Name("Compares the entity name", "String"), Group("Compares the entity group. This is made-up. Use entity list to see what comes up", "String"),

		Class("Compares the entity class", "Class name. Ex: EntityPig"), LClass("Compares the package and entity name", "Full class name. Ex: net.minecraft.passive.EntityPig"), Instance("Checks if the entity class/superclass is the one specified", "Full class name. Ex: net.minecraft.entity.item.EntityItem"),

		Dimension("Checks the entity dimension", "Integer list, <n>[.<n>[.<n>...]]. Ex: -1.0.1"), Where("Checks the entity chunk coordinates to be between", "Chunk coords, <x>.<z>[/<x2>.<z2>]. Ex:-10.30"), // chunk
																																																				// location
		Pos("Checks the entity position", "Coords, <x>.<y>.<z>[/<x2>.<y2>.<z2>]. Ex:-500.0.-500/500.255.500"), Hash("Entity java given hashcode", "Hash. Ex: 4d4a2b4d");
	...
	private boolean valueStringEquals(String other) {
		return value.equalsIgnoreCase(other.replaceAll(" ", "_")); // TODO:
																	// implement
																	// some regex
	}

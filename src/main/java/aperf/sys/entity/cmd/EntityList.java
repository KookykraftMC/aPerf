...
public class EntityList extends BaseCommand {
	@Command(name = "aperf", syntax = "(?:entity|e)", description = "Lists the entity types and counts", isPrimary = true, permission = "aperf.cmd.entity.list")
	public void entry(Object plugin, ICommandSender sender, Map<String, String> args) throws Exception {
		list(plugin, sender, null);
	}

	@Command(name = "aperf", syntax = "(?:entity|e) (?:listhere|lh) [group] [filter] [limit]", description = "Lists the entities at your chunk\n" + "Group: group/name/class/lclass/where/pos\n" + "Filter: use /ap filterhelp\n", permission = "aperf.cmd.entity.listhere", isPlayerOnly = true)
	...
	@Command(name = "aperf", syntax = "(?:entity|e) (?:listnearhere|lnh) <radius> [group] [filter] [limit]", description = "Lists the entities near your chunk\n" + "Group: group/name/class/lclass/where/pos\n" + "Filter: use /ap filterhelp\n", permission = "aperf.cmd.entity.listnearhere", isPlayerOnly = true)
	...
	@Command(name = "aperf", syntax = "(?:entity|e) (?:listaround|la) <radius> [group] [filter] [limit]", description = "Lists the entities around you\n" + "Radius: number of blocks around you\n" + "Group: group/name/class/lclass/where/pos\n" + "Filter: use /ap filterhelp\n", permission = "aperf.cmd.entity.listaround", isPlayerOnly = true)
	...
	@Command(name = "aperf", syntax = "(?:entity|e) (?:list|l) [group] [filter] [limit]", description = "Lists the entities\n" + "Group: group/name/class/lclass/where/pos\n" + "Filter: use /ap filterhelp\n", permission = "aperf.cmd.entity.list")
	public void list(Object plugin, ICommandSender sender, Map<String, String> args) throws Exception {
		...
		sendWorldGroupedList("Entity list grouped by " + sGrp + (filter != null ? ", filtered by " + filter.serializeDisplay() : "") + (limitCnt != null ? ", limited by " + args.get("limit").trim() : ""), sender, new IListForObject<WorldServer>() {
			@Override

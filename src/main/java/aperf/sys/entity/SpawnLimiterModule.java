...
public class SpawnLimiterModule extends ModuleBase {
    ...
    public List<Long> fullyLoadedChunks = new ArrayList<Long>(); // managed by
                                                                 // events
    ...
    public SpawnLimiterModule() {
        addCommand(new aperf.sys.entity.cmd.SpawnLimiterCmd());
    }
    ...
    private void indexLoadedChunks() {
        try {
            synchronized (fullyLoadedChunks) {
                ...
                Field f = ChunkProviderServer.class.getDeclaredField("field_73245_g");
                f.setAccessible(true); // private List loadedChunks = new
                                       // ArrayList();

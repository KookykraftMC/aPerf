...
public class Status extends BaseCommand {
    ...
    public void status(Object plugin, ICommandSender sender, Map<String, String> args) {
        ...
        for (WorldServer w : MinecraftServer.getServer().worldServers) {
            ...
            msg(sender, format, ChatColor.GREEN, dim, prc_tick >= 100 ? ChatColor.DARK_GREEN : prc_tick >= 50 ? ChatColor.GOLD : ChatColor.RED, (prc_tick > 999 ? ">" : "") + String.valueOf(prc_tick > 999 ? 999 : prc_tick) + "%", ChatColor.GREEN, w.playerEntities.size(), w.loadedEntityList.size(), w.loadedTileEntityList.size(), w.getChunkProvider().getLoadedChunkCount(), forced.keySet().size());
        }
    ...
    public void forced(Object plugin, ICommandSender sender, Map<String, String> args) throws Exception {
        ...
        for (WorldServer serv : MinecraftServer.getServer().worldServers) {
            ...
            msg(sender, "%s%s [%d], %s tickets", ChatColor.GREEN, serv.provider.getDimensionName(), serv.provider.dimensionId, tickets.size());
            ...
            for (Ticket ticket : tickets) {
                ...
                msg(sender, "%s  %s, %s, %s, %s, \"%s\" - %s of %s chunks", ChatColor.GOLD, ticket.getType(), ticket.getModId(), ticket.getPlayerName(), ticket.getEntity(), Joiner.on("|").join(vals), ticket.getChunkList().size(), ticket.getChunkListDepth());
                ...
                for (Object pos : ticket.getChunkList()) {
                     ...
                     * p.chunkXPos, p.chunkZPos, p.getCenterXPos(),
                     * p.getCenterZPosition());
                     */
                }

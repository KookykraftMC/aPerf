...
public class PacketManagerModule extends ModuleBase implements IPacketHandler {
	...
	public PacketManagerModule() {
		addCommand(new aperf.sys.packet.cmd.PacketStats());
		addCommand(new aperf.sys.packet.cmd.PacketLimits());
	...
	public ArrayList<SimpleEntry<SimpleEntry<String, Integer>, Double>> getTestResults(EntityPlayer player) {
		...
		for (String tag : stat.keySet()) {
			for (int i = 0; i < stat.get(tag).length; i++) {
				if (stat.get(tag)[i] > 0) {
					ret.add(new SimpleEntry<SimpleEntry<String, Integer>, Double>(new SimpleEntry<String, Integer>(tag, i), stat.get(tag)[i] / time));
				}
			}
		}
	...
	public ArrayList<SimpleEntry<Class<?>, Double>> getTestResultsAbstract(EntityPlayer player) {
		...
		for (Class<?> key : stat.keySet()) {
			...
			if (used > 0) {
				ret.add(new SimpleEntry<Class<?>, Double>(key, used / time));
			}
		}
	...
	public void hook() {
		...
		if (!hooked) {
			...
			try {
				...
				throw new RuntimeException("Could not attach the packet listener. Perhaps rerun the patcher?", t);
			}
	...
	public static String getPacketDesc(String tag, int subId) {
		if (tag.equals("rp2")) {
			...
			if (subId == 0) {
				...
				return "EE Block update (coord, dir, player name)"; // server ->
			} else if (subId == 2) {

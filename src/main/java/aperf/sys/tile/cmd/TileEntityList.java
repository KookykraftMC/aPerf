...
public class TileEntityList extends BaseCommand {
	@Command(name = "aperf", syntax = "(?:tile|t)", description = "Lists the tile types and counts", isPrimary = true, permission = "aperf.cmd.tile.list")
	public void entry(Object plugin, ICommandSender sender, Map<String, String> args) throws Exception {
		list(plugin, sender, null);
	}

	@Command(name = "aperf", syntax = "(?:tile|t) (?:listhere|lh) [group] [filter] [limit]", description = "Lists the tiles at your chunk\n" + "Group: group/name/class/lclass/where/pos\n" + "Filter: use /ap filterhelp\n", permission = "aperf.cmd.tile.listhere", isPlayerOnly = true)
	...
	@Command(name = "aperf", syntax = "(?:tile|t) (?:listnearhere|lnh) <radius> [group] [filter] [limit]", description = "Lists the tiles near your chunk\n" + "Group: group/name/class/lclass/where/pos\n" + "Filter: use /ap filterhelp\n", permission = "aperf.cmd.tile.listnearhere", isPlayerOnly = true)
	...
	@Command(name = "aperf", syntax = "(?:tile|t) (?:listaround|la) <radius> [group] [filter] [limit]", description = "Lists the tiles around you\n" + "Radius: number of blocks around you\n" + "Group: group/name/class/lclass/where/pos\n" + "Filter: use /ap filterhelp\n", permission = "aperf.cmd.tile.listaround", isPlayerOnly = true)
	public void listaround(Object plugin, ICommandSender sender, Map<String, String> args) throws Exception {
		...
		filter = filter + String.format("d:%d,p:%d.%d.%d/%d.%d.%d", p.dimension, (int) p.posX - radius, (int) p.posY - radius, (int) p.posZ - radius, (int) p.posX + radius, (int) p.posY + radius, (int) p.posZ + radius);
		args.put("filter", filter);

		list(plugin, sender, args);
	}

	@Command(name = "aperf", syntax = "(?:tile|t) (?:list|l) [group] [filter] [limit]", description = "Lists the tiles\n" + "Group: group/name/class/lclass/where/pos\n" + "Filter: use /ap filterhelp\n", permission = "aperf.cmd.tile.list")
	public void list(Object plugin, ICommandSender sender, Map<String, String> args) throws Exception {
		...
		sendWorldGroupedList("Tile list grouped by " + sGrp + (iFilter != null ? ", filtered by " + iFilter.serializeDisplay() : "") + (limitCnt != null ? ", limited by " + args.get("limit").trim() : ""), sender, new IListForObject<WorldServer>() {
			@Override
			...
			public String group(TileEntity ent) {
				...
				if (iGrp == 1) {
					...
					return String.format("%s/%s @ %d,%d,%d", TileEntityHelper.getEntityName(ent), Integer.toHexString(System.identityHashCode(ent)), ent.xCoord, ent.yCoord, ent.zCoord);
				} else {
